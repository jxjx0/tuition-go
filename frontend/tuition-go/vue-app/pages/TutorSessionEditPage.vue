      ...
      <template v-else>
        ...
        <div v-if="isCompleted" class="space-y-5">
          ...
          <div class="rounded-2xl p-6" style="background-color:#fff;border:1px solid #E8F0FE">
            ...
            <div v-if="session.review" class="flex items-center gap-3">
              <img
                :src="avatarUrl(session.studentImageUrl, session.studentId)"
              ...
              <div class="flex flex-col gap-1">
                <div class="flex items-center gap-0.5">
                  <template v-for="i in 5" :key="i">
                    <svg class="w-4 h-4" :style="i <= session.review.rating ? 'color:#F59E0B' : 'color:#E8F0FE'" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                  </template>
                </div>
                <p class="text-sm" style="color:#1B3A5C;opacity:0.8">{{ session.review.comment }}</p>
              </div>
            </div>
            <div v-else class="flex flex-col items-center gap-2 py-6 rounded-xl" style="background-color:#F5F7FA">
