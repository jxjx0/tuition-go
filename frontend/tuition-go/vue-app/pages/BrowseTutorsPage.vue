...
const allSubjects = computed(() => { 
  ...
  mockTutors.forEach(t => t.subjects.forEach(su => s.add(su.name)))
  return Array.from(s).sort() 
})

const allLevels = computed(() => { 
  const l = new Set<string>()
  mockTutors.forEach(t => t.subjects.forEach(su => l.add(su.level)))
...
const filteredTutors = computed(() => {
  ...
  if (searchQuery.value) { 
    ...
    result = result.filter(t => t.name.toLowerCase().includes(q) || t.subjects.some(s => s.name.toLowerCase().includes(q))) 
  }
          ...
          <div class="p-6">
            <div class="flex items-start gap-4">
              ...
              <div class="text-right flex-shrink-0">
                <p class="text-xl font-extrabold" style="color:#2EAA4F">${{ tutor.hourlyRate }}</p>
                <p class="text-xs" style="color:#1B3A5C;opacity:0.5">/hour</p>
              </div>
            </div>
            <div class="mt-4 flex flex-wrap gap-2">
              <span v-for="sub in tutor.subjects.slice(0,3)" :key="sub.name+sub.level" class="px-2.5 py-1 rounded-lg text-xs font-medium" style="background-color:#E8F0FE;color:#4A90D9">{{ sub.name }} ({{ sub.level }})</span>
