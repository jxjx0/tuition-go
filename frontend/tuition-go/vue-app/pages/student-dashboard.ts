...
export const StudentDashboardPage = defineComponent({
  ...
  setup() {
    ...
    return { activeTab, upcomingSessions, pastSessions, cancelledSessions, tabs, dashStats, fmtDate }
  },
  template: `
    <div class="py-8 md:py-12" style="background-color:#F5F7FA">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-10">
          <div><h1 class="text-3xl font-extrabold" style="color:#1B3A5C">Welcome back, Alex</h1><p class="mt-1 text-base" style="color:#1B3A5C;opacity:0.6">Here is a summary of your learning journey</p></div>
        ...
        <div v-if="activeTab==='upcoming'" class="space-y-4">
          <div v-for="session in upcomingSessions" :key="session.id" class="rounded-2xl border p-5 hover:shadow-sm" style="background-color:#fff;border-color:#E8F0FE">
            <div class="flex flex-col sm:flex-row items-start gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap"><h3 class="text-base font-bold" style="color:#1B3A5C">{{ session.subject }} ({{ session.level }})</h3><span class="px-2.5 py-0.5 rounded-full text-xs font-semibold" style="background-color:#E8F0FE;color:#4A90D9">Upcoming</span></div>
                <p class="text-sm mt-1" style="color:#1B3A5C;opacity:0.7">with {{ session.tutorName }}</p>
                <div class="flex flex-wrap items-center gap-4 mt-3 text-xs" style="color:#1B3A5C;opacity:0.6"><span>{{ fmtDate(session.date) }}</span><span>{{ session.startTime }} - {{ session.endTime }}</span><span class="font-semibold" style="color:#2EAA4F">\${{ session.price.toFixed(2) }}</span></div>
          ...
          <div v-if="!upcomingSessions.length" class="text-center py-16 rounded-2xl border" style="background-color:#fff;border-color:#E8F0FE"><p class="text-base font-semibold" style="color:#1B3A5C">No upcoming sessions</p><router-link to="/tutors" class="inline-block mt-4 px-6 py-2.5 rounded-xl text-sm font-semibold text-white" style="background-color:#4A90D9">Find Tutors</router-link></div>
        </div>
        <div v-if="activeTab==='past'" class="space-y-4">
          <div v-for="session in pastSessions" :key="session.id" class="rounded-2xl border p-5 hover:shadow-sm" style="background-color:#fff;border-color:#E8F0FE">
            <div class="flex flex-col sm:flex-row items-start gap-4">
              <img :src="session.tutorAvatar" :alt="session.tutorName" class="w-14 h-14 rounded-xl object-cover flex-shrink-0" crossorigin="anonymous" style="background-color:#E8F0FE"/>
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap"><h3 class="text-base font-bold" style="color:#1B3A5C">{{ session.subject }} ({{ session.level }})</h3><span class="px-2.5 py-0.5 rounded-full text-xs font-semibold" style="background-color:rgba(46,170,79,0.1);color:#2EAA4F">Completed</span></div>
                <p class="text-sm mt-1" style="color:#1B3A5C;opacity:0.7">with {{ session.tutorName }}</p>
                <div class="flex flex-wrap items-center gap-4 mt-2 text-xs" style="color:#1B3A5C;opacity:0.6"><span>{{ fmtDate(session.date) }}</span><span>{{ session.startTime }} - {{ session.endTime }}</span><span class="font-semibold" style="color:#2EAA4F">\${{ session.price.toFixed(2) }}</span></div>
              </div>
        ...
        <div v-if="activeTab==='cancelled'" class="space-y-4">
          <div v-for="session in cancelledSessions" :key="session.id" class="rounded-2xl border p-5 opacity-70" style="background-color:#fff;border-color:#E8F0FE">
            <div class="flex flex-col sm:flex-row items-start gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 flex-wrap"><h3 class="text-base font-bold" style="color:#1B3A5C">{{ session.subject }} ({{ session.level }})</h3><span class="px-2.5 py-0.5 rounded-full text-xs font-semibold" style="background-color:rgba(239,68,68,0.1);color:#ef4444">Cancelled</span></div>
                <p class="text-sm mt-1" style="color:#1B3A5C;opacity:0.7">with {{ session.tutorName }}</p>
                <p class="text-xs mt-2" style="color:#1B3A5C;opacity:0.6">{{ fmtDate(session.date) }} &middot; {{ session.startTime }} - {{ session.endTime }}</p>
              </div>
            </div>
          </div>
          <div v-if="!cancelledSessions.length" class="text-center py-16 rounded-2xl border" style="background-color:#fff;border-color:#E8F0FE"><p class="text-sm" style="color:#1B3A5C;opacity:0.6">No cancelled sessions</p></div>
        </div>
      </div>
    </div>
  `,
