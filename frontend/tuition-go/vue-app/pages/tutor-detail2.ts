...
export const TutorDetailPage2 = defineComponent({
  ...
  setup() {
    ...
    return { tutor, loading, availableSessions, tutorReviews, formatDate }
  },
  template: `
    <div class="py-8 md:py-12" style="background-color:#F5F7FA">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8" v-if="tutor">
        ...
        <div class="rounded-2xl border overflow-hidden mb-8" style="background-color:#fff;border-color:#E8F0FE">
          <div class="p-6 md:p-8" style="background:linear-gradient(135deg,#1B3A5C 0%,#4A90D9 100%)">
            <div class="flex flex-col md:flex-row items-start gap-6">
              <div class="relative flex-shrink-0"><img :src="tutor.imageURL" :alt="tutor.name" class="w-24 h-24 md:w-28 md:h-28 rounded-2xl object-cover border-4 border-white shadow-lg" crossorigin="anonymous" style="background-color:#E8F0FE"/><div v-if="tutor.verified" class="absolute -bottom-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center shadow-sm" style="background-color:#2EAA4F"><svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg></div></div>
              <div class="flex-1"><h1 class="text-2xl md:text-3xl font-extrabold text-white">{{ tutor.name }}</h1><div class="flex flex-wrap items-center gap-4 mt-3"><div class="flex items-center gap-1"><StarRating :modelValue="tutor.averageRating" size="sm"/><span class="text-sm font-bold text-white ml-1">{{ tutor.averageRating }}</span><span class="text-sm" style="color:rgba(255,255,255,0.7)">({{ tutor.totalReviews }} reviews)</span></div></div><div class="flex flex-wrap gap-2 mt-4"><span v-for="sub in tutor.subjects" :key="sub.subject + sub.academicLevel" class="px-3 py-1 rounded-lg text-xs font-medium" style="background-color:rgba(255,255,255,0.2);color:#fff">{{ sub.subject }} ({{ sub.academicLevel }})</span></div></div>
          ...
          <div class="p-6 md:p-8">
            ...
            <div class="mb-8"><h2 class="text-lg font-bold mb-3" style="color:#1B3A5C">Hourly Rates Based on Subjects (Academic Level)</h2><div class="overflow-hidden rounded-xl border" style="border-color:#E8F0FE"><table class="w-full text-sm"><thead><tr style="background-color:#E8F0FE"><th class="text-left px-4 py-3 font-semibold" style="color:#1B3A5C">Subject</th><th class="text-left px-4 py-3 font-semibold" style="color:#1B3A5C">Academic Level</th><th class="text-right px-4 py-3 font-semibold" style="color:#1B3A5C">Hourly Rate</th></tr></thead><tbody><tr v-for="(sub, idx) in tutor.subjects" :key="sub.tutorSubjectId" class="border-t" :style="{ borderColor:'#E8F0FE', backgroundColor: idx % 2 === 0 ? '#fff' : '#F5F7FA' }"><td class="px-4 py-3 font-medium" style="color:#1B3A5C">{{ sub.subject }}</td><td class="px-4 py-3" style="color:#1B3A5C;opacity:0.8">{{ sub.academicLevel }}</td><td class="px-4 py-3 text-right font-bold" style="color:#2EAA4F">\${{ sub.hourlyRate }}/hr</td></tr></tbody></table></div></div>
          </div>
        </div>
        ...
        <div class="rounded-2xl border p-6 md:p-8" style="background-color:#fff;border-color:#E8F0FE">
          <div class="flex items-center justify-between mb-6"><h2 class="text-lg font-bold" style="color:#1B3A5C">Student Reviews</h2><span class="text-sm font-medium px-3 py-1 rounded-lg" style="background-color:#E8F0FE;color:#4A90D9">{{ tutor.totalReviews }} reviews</span></div>
