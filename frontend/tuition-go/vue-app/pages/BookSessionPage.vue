  ...
  <div class="py-8 md:py-12" style="background-color:#F5F7FA">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="session&&!confirmed" class="rounded-2xl border overflow-hidden" style="background-color:#fff;border-color:#E8F0FE">
        <div class="p-6 border-b" style="border-color:#E8F0FE;background:linear-gradient(135deg,#1B3A5C 0%,#4A90D9 100%)">
          <h1 class="text-2xl font-extrabold text-white">Confirm Your Booking</h1>
          <p class="text-sm mt-1" style="color:rgba(255,255,255,0.75)">Review the session details before confirming</p>
        </div>
        <div class="p-6 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center justify-between py-3 border-b" style="border-color:#E8F0FE">
              ...
              <span class="text-sm font-semibold" style="color:#1B3A5C">{{ session.startTime }} - {{ session.endTime }}</span>
            </div>
            <div class="flex items-center justify-between py-3 border-b" style="border-color:#E8F0FE">
              <span class="text-sm" style="color:#1B3A5C;opacity:0.7">Duration</span>
              <span class="text-sm font-semibold" style="color:#1B3A5C">{{ session.duration }} minutes</span>
            </div>
            <div class="flex items-center justify-between py-3">
              <span class="text-sm font-bold" style="color:#1B3A5C">Total</span>
              <span class="text-2xl font-extrabold" style="color:#2EAA4F">${{ session.price.toFixed(2) }}</span>
            </div>
          </div>
          <div class="rounded-xl p-5 border" style="border-color:#E8F0FE">
            <h3 class="text-sm font-bold mb-3" style="color:#1B3A5C">Payment Method</h3>
            <div class="p-4 rounded-xl text-center" style="background-color:#F5F7FA;border:2px dashed #E8F0FE">
              <p class="text-xs font-medium" style="color:#1B3A5C;opacity:0.6">Stripe Checkout integration</p>
              <p class="text-xs mt-1" style="color:#4A90D9">Secure payment processing</p>
            </div>
          </div>
        ...
        <div v-if="session" class="rounded-xl p-5 mb-6 text-left" style="background-color:#F5F7FA">
          <div class="space-y-3">
            <div class="flex justify-between text-sm">
              <span style="color:#1B3A5C;opacity:0.7">Tutor</span>
              <span class="font-semibold" style="color:#1B3A5C">{{ session.tutorName }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span style="color:#1B3A5C;opacity:0.7">Subject</span>
              <span class="font-semibold" style="color:#1B3A5C">{{ session.subject }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span style="color:#1B3A5C;opacity:0.7">Meeting Link</span>
              <a href="https://meet.google.com/abc-defg-hij" target="_blank" class="font-semibold underline" style="color:#4A90D9">Join Google Meet</a>
            </div>
          </div>
        </div>
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          <router-link to="/dashboard" class="px-6 py-3 rounded-xl text-sm font-semibold text-white" style="background-color:#4A90D9">Go to Dashboard</router-link>
          <router-link to="/tutors" class="px-6 py-3 rounded-xl text-sm font-semibold border" style="border-color:#E8F0FE;color:#1B3A5C">Browse More Tutors</router-link>
        </div>
      </div>
