    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8" v-if="tutor">
      ...
      <div class="rounded-2xl border overflow-hidden mb-8" style="background-color:#fff;border-color:#E8F0FE">
        <div class="p-6 md:p-8" style="background:linear-gradient(135deg,#1B3A5C 0%,#4A90D9 100%)">
          <div class="flex flex-col md:flex-row items-start gap-6">
            <div class="relative flex-shrink-0">
              <img :src="tutor.avatar" :alt="tutor.name" class="w-24 h-24 md:w-28 md:h-28 rounded-2xl object-cover border-4 border-white shadow-lg" crossorigin="anonymous" style="background-color:#E8F0FE"/>
            ...
            <div class="flex-1">
              ...
              <div class="flex flex-wrap items-center gap-4 mt-3">
                <div class="flex items-center gap-1">
                  <StarRating :modelValue="tutor.rating" size="sm"/>
                  <span class="text-sm font-bold text-white ml-1">{{ tutor.rating }}</span>
                  <span class="text-sm" style="color:rgba(255,255,255,0.7)">({{ tutor.totalReviews }} reviews)</span>
                </div>
                <span class="text-sm" style="color:rgba(255,255,255,0.7)">{{ tutor.totalSessions }} sessions</span>
              </div>
              <div class="flex flex-wrap gap-2 mt-4">
                <span v-for="sub in tutor.subjects" :key="sub.name+sub.level" class="px-3 py-1 rounded-lg text-xs font-medium" style="background-color:rgba(255,255,255,0.2);color:#fff">{{ sub.name }} ({{ sub.level }})</span>
              </div>
            </div>
            <div class="flex-shrink-0 text-center md:text-right">
              <p class="text-3xl font-extrabold text-white">${{ tutor.hourlyRate }}</p>
              <p class="text-sm" style="color:rgba(255,255,255,0.7)">per hour</p>
            </div>
          </div>
        </div>
        <div class="p-6 md:p-8">
          ...
          <div class="mb-8">
            <h2 class="text-lg font-bold mb-3" style="color:#1B3A5C">Qualifications</h2>
            <div class="flex flex-wrap gap-2">
              <span v-for="q in tutor.qualifications" :key="q" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium" style="background-color:#E8F0FE;color:#4A90D9">
          ...
          <div class="mb-8">
            <h2 class="text-lg font-bold mb-3" style="color:#1B3A5C">Teaching Style</h2>
            <p class="text-sm leading-relaxed" style="color:#1B3A5C;opacity:0.8">{{ tutor.teachingStyle }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-2xl border p-6 md:p-8" style="background-color:#fff;border-color:#E8F0FE">
        <div class="flex items-center justify-between mb-6">
          ...
          <span class="text-sm font-medium px-3 py-1 rounded-lg" style="background-color:#E8F0FE;color:#4A90D9">{{ tutorReviews.length }} reviews</span>
        </div>
